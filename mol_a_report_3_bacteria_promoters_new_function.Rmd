...
for (i in funcs){
  assign(paste0(i,' _E01_e.coli_promoters'), apply(ecoli_E01, MARGIN = 1, FUN = i))
}
...
for (i in funcs){
  assign(paste0(i,'_d1_e.coli_promoters'), apply(ecoli_d1, MARGIN = 1, FUN = i))
}
...
for (i in funcs){
  assign(paste0(i,'_GC_e.coli_promoters'), apply(ecoli_gc, MARGIN = 1, FUN = i))
}
...
for (i in funcs){
  assign(paste0(i,'_EP_e.coli_promoters'), apply(ecoli_gc, MARGIN = 1, FUN = i))
}
...
```{r GO statistics using GOstats, warning=FALSE, message=FALSE, cache=TRUE, eval=FALSE}
library(GOstats)
library(Category)
library(DBI)
library(org.EcK12.eg.db)
