...
library(factoextra)
fviz_pca_biplot(PCA12, col.var = 2)+ggtitle('PCA result on hexamers occurences')
```

```{r hexamers occurences PCA, pca 3-4}
PCA34 <- prcomp(all_seqs_oligos_freq[,c(3,4)], scale = T)
fviz_pca_biplot(PCA34, col.var = 2)+ggtitle('PCA result on hexamers occurences')
```

```{r hexamers occurences PCA, pca 1-3}
PCA13 <- prcomp(all_seqs_oligos_freq[,c(1,3)], scale = T)
fviz_pca_biplot(PCA13, col.var = 2)+ggtitle('PCA result on hexamers occurences')
```
...
plot(ward_b.subtilis_proms_non_proms_mpots, labels = F)
...
plot(ward_e.coli_b.subtilis_proms_mpots, labels = F)
...
plot(ward_e.coli_proms_non_proms_bends, labels = F)
...
plot(ward_b.subtilis_proms_non_proms_bends, labels = F)
...
plot(ward_e.coli_b.subtilis_proms_bends, labels = F)
