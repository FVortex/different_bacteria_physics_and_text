...
```{r All the vlhA}
vlha_600 <- read.fasta('/home/mikhail/Documents/mol_a_2017/vlhA_600nts.fasta', seqonly = F, as.string = T)
length(vlha_60)
str(vlha_600)
...
labels <- -rev(1:nchar(vlha$HFMG94VAA_RS01210))
labels[seq(1, nchar(vlha$HFMG94VAA_RS01210), by = 25)]

labels_new <- rep('', length(labels))
labels_new[seq(1, nchar(vlha$HFMG94VAA_RS01210), by = 25)] <- labels[seq(1, nchar(vlha$HFMG94VAA_RS01210), by = 25)]

ggseqlogo(sapply(vlha, FUN = function(x) {paste0(toupper(x), collapse = '')}),  ncol = 1)+ scale_y_continuous(name="Информационнное содержание (бит)")+scale_x_discrete(name ="Последовательность относительно начала кодирующей последовательности (п.н.)", limits=1:nchar(vlha$HFMG94VAA_RS01210), labels =labels_new)+coord_fixed(ratio = 40)


boxplot(lapply(vlha, GC))



dyn_int <- 25

Ldyn <- lapply(vlha, FUN = function(x) {dynchars(seq = strsplit(x, split = '')[[1]], average_interval_size = dyn_int, tss = 190, boundaries = c(160, 70), strand = 'forward')})
`Sequence (nts)` <- -rev(seq_along(Ldyn$HFMG94VAA_RS01210$E0))
#E0
mat_dyn <- sapply(Ldyn, function(x) {x$E0})
means_to_mat <- c(NA, apply(na.omit(mat_dyn),  MARGIN = 1, FUN = median), NA)
matplot(`Sequence (nts)`, mat_dyn, type = 'l', lwd = 0.25,  col = 1, ylab = 'Open states activation energy (kcal/mole)', main = 'Open states activation energy (sliding window 25 nts)\n for 441 vlhA sequences')
lines(`Sequence (nts)`, means_to_mat, col = 'magenta', lwd = 2)

gaa_repeats <- -155:-130
approx_tsss <- max(gaa_repeats)+50

segments(x0 = min(gaa_repeats), y0 = 275, x1 = max(gaa_repeats), y1 = 275, lwd = 3, col = 'darkred')
# #arrows(x0 = approx_tsss, y0 = 280, x1 = approx_tsss, y1 = 255, col = 'darkred', lwd = 3)
abline(v = approx_tsss, lty=2, lwd = 2)
legend('topright', legend = c('GAA-repeats', 'Approximate TSS', 'Median values'), lty = c(1,2,1), col = c('darkred', 'black', 'magenta'))

#gc

mat_dyn <- sapply(Ldyn, function(x) {x$gc})
means_to_mat <- c(NA, apply(na.omit(mat_dyn),  MARGIN = 1, FUN = median), NA)
matplot(`Sequence (nts)`, mat_dyn, type = 'l', lwd = 0.25,  col = 1, ylab = 'GC-content (25nts-long sliding window)', main = 'GC-content (25nts-long sliding window)\n for 441 vlhA sequences')
lines(`Sequence (nts)`, means_to_mat, col = 'magenta', lwd = 2)
...
Lbend <- lapply(vlha, FUN = function(x) {bendability(s = x, bound =  c(1, nchar(vlha$HFMG94VAA_RS01210)), width = 1)})
`Sequence (nts)` <- -rev(seq_along(Lbend$HFMG94VAA_RS01210))
#E0
mat_bend <- sapply(Lbend, function(x) {x})
means_to_mat <- c(NA, NA, apply(na.omit(mat_bend),  MARGIN = 1, FUN = median), NA, NA, NA)
matplot(`Sequence (nts)`, mat_bend, type = 'l', lwd = 0.25,  col = 1, ylab = 'Bendability', main = 'Bendability\n for 441 vlhA sequences')
lines(`Sequence (nts)`, means_to_mat, col = 'magenta', lwd = 2)
...
Lep <- lapply(vlha, FUN = function(x) {
  p <- lseqspline1D(s = x, bound = c(1, nchar(vlha$HFMG94VAA_RS01210)), width = 1, ref = 190)
(p$mpot[p$x %in% zout])
})
`Sequence (angstrom)` <- -rev(seq_along(Lep$HFMG94VAA_RS01210))
#E0
mat_ep <- sapply(Lep, function(x) {x})
means_to_mat <- c(apply(na.omit(mat_ep),  MARGIN = 1, FUN = median))
matplot(`Sequence (angstrom)`, mat_ep, type = 'l', lwd = 0.25,  col = 1, ylab = 'Electrostatic potential', main = 'Electrostatic potential\n for 441 vlhA sequences')
lines(`Sequence (angstrom)`, means_to_mat, col = 'magenta', lwd = 2)

gaa_repeats <- -155:-130*2.75
approx_tsss <- -80*2.75

segments(x0 = min(gaa_repeats), y0 = max(mat_ep, na.rm = T), x1 = max(gaa_repeats), y1 =  max(mat_ep, na.rm = T), lwd = 3, col = 'darkred')
# #arrows(x0 = approx_tsss, y0 = 280, x1 = approx_tsss, y1 = 255, col = 'darkred', lwd = 3)
abline(v = approx_tsss, lty=2, lwd = 2)
legend('topright', legend = c('GAA-repeats', 'Approximate TSS', 'Median values'), lty = c(1,2,1), col = c('darkred', 'black', 'magenta'))
...
for (i in seq_along(mycoplasma_proms$Position)){
  if (mycoplasma_proms$Strand[i] == '1'){ #initial strand name
   tmp <- substr(mycoplasma_CP006916.3, start = mycoplasma_proms$Position[i]-200, stop = mycoplasma_proms$Position[i]+200)
  } else {
    tmp <- substr(complement_mycoplasma_CP006916.3, start = mycoplasma_proms$Position[i]-200, stop = mycoplasma_proms$Position[i]+200)
   }
...
which(sapply(mycoplasma_strings, nchar)!=401)
#351

mycoplasma_strings <- mycoplasma_strings[-which(sapply(mycoplasma_strings, nchar)!=401)]
