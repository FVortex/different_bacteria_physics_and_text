...
```{r All the vlhA}
vlha_600 <- read.fasta('/home/mikhail/Documents/mol_a_2017/Vlh_600.fasta', seqonly = F, as.string = T)
length(vlha_600)
str(vlha_600)
...
Ldyn <- lapply(vlha_600, FUN = function(x) {dynchars(seq = strsplit(x, split = '')[[1]], average_interval_size = dyn_int, tss = 190, boundaries = c(160, 70), strand = 'forward')})
`Sequence (nts)` <- -rev(seq_along(Ldyn$HFMG94VAA_RS01210$E0))
#E0
mat_dyn <- sapply(Ldyn, function(x) {x$E0})
means_to_mat <- c( apply(na.omit(mat_dyn),  MARGIN = 1, FUN = median))
...
segments(x0 = min(gaa_repeats), y0 = max(mat_dyn), x1 = max(gaa_repeats), y1 = max(mat_dyn), lwd = 3, col = 'darkred')
...
Lbend <- lapply(vlha_600, FUN = function(x) {bendability(s = x, bound =  c(1, nchar(vlha_600$HFMG94VAA_RS01210)), width = 1)})
...
matplot(`Sequence (nts)`, mat_bend, type = 'l', lwd = 0.09,  col = 1, ylab = 'Bendability', main = 'Bendability\n for 441 vlhA sequences')
lines(`Sequence (nts)`, means_to_mat, col = 'magenta', lwd = 2)
...
for (i in seq_along(mycoplasma_proms$Position)){
  if (mycoplasma_proms$Strand[i] == '1'){ #initial strand name
   tmp <- substr(mycoplasma_CP006916.3, start = mycoplasma_proms$Position[i]-520, stop = mycoplasma_proms$Position[i]+80) #to compare with intervals
    ...
    tmp <- substr(complement_mycoplasma_CP006916.3, start = mycoplasma_proms$Position[i]-520, stop = mycoplasma_proms$Position[i]+80)
   }
...
mycoplasma_chars <- lapply(mycoplasma_strings, FUN = function(x) {strsplit(x, split = '')[[1]]})
...
which(str_detect(vlha_600, pattern = '[^(gaa)](gaa){12}(?!(gaa))'))
...
ggseqlogo(sapply(vlha_600[exactlty12], FUN = function(x) {paste0(toupper(x), collapse = '')}),  ncol = 1)+ scale_y_continuous(name="Информационнное содержание (бит)")+scale_x_discrete(name ="Последовательность относительно начала кодирующей последовательности (п.н.)", limits=1:nchar(vlha_600$HFMG94VAA_RS01210), labels =labels_new)+coord_fixed(ratio = 40)
