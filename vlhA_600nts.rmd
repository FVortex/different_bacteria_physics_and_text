...
mycoplasma_strings_major_cluster <- mycoplasma_strings[which(cut_ward_mycoplasma_proms_stringdist !=3 )]
...
Lbend <- lapply(mycoplasma_strings_major_cluster, FUN = function(x) {bendability(s = x, bound =  c(600-250, nchar(vlha_600$HFMG94VAA_RS01210)), width = 1)})
...
#EP
zout <- -(540/1.5):(270/1.5)
Lep <- lapply(mycoplasma_strings_major_cluster, FUN = function(x) {
  p <- lseqspline1D(s = x, bound = c(1, nchar(vlha_600$HFMG94VAA_RS01210)), width = 1, ref = 600-80)
(p$mpot[p$x %in% zout]) #!boundary
})
#EP
