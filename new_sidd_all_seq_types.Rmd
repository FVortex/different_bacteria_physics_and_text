...
svg('/home/jane/Документы/Misha/mol_a_2018/mce2018_oligos_pca_seq_types.svg', height = 10, width = 8)
fviz_pca_biplot(PCA, 
             axes = c (1,2),
...
#same but cluster abline on distinct plots

# #svg('/home/jane/Документы/Misha/mol_a_2018/masterplot_compact_sidd_cluster_location_quadrants.svg', height = 10, width = 10)
#masterplot
layout((matrix( c(1,2,3,4,7,1,2,3,5,7,1,2,3,6,7), ncol=3)))
par(mar = rep(0.7,4))
...
plot(1:nchar(e.coli_U00096.2), type = 'n', ylim = c(-1,1), main = 'Compact clusters TSS and OriC/terC location ', yaxt = 'n', ylab = '', xlab = 'Genome (nts)')
abline(v = exptsss[which(cut_ward_expsidds==2)], col = 'darkred', lwd = lwd)
abline(v = c(mean(rep_origin), mean(rep_terminus)), col = 'black', lwd = 2)
...
#complete forward strand result
library(gtools)
dir_predictions <- mixedsort(dir('/home/jane/Документы/Misha/mol_a_2018/predictions/'))

forward_strand_all_predictions <- c()
for (i in dir_predictions) {
  
 tmp <- get(load(paste0('/home/jane/Документы/Misha/mol_a_2018/predictions/', i)))
   forward_strand_all_predictions <- c(forward_strand_all_predictions, tmp$Promoter)
}

summary(forward_strand_all_predictions)

which_over095 <- which(forward_strand_all_predictions > 0.99)


#constructing iranges, left flank is expanded by 200 nts
...
seq_margs <- seq(100, 500, 100)
seq_cutoff <- c(0.75, 0.85, 0.95, 0.99)

df <- as.data.frame(matrix(NA, length(seq_margs), length(seq_cutoff )))

colnames(df) <- seq_cutoff
rownames(df) <- seq_margs

df1 <- df

for (marg in seq_along(seq_margs)) {
  for (i in seq_along(seq_cutoff)){
#marg <- 100
    tmp_which_over_i <- which(forward_strand_all_predictions > seq_cutoff[i])
    expIR <- reduce(IRanges(start = exptsss[which.forward]-seq_margs[marg], end = exptsss[which.forward]+seq_margs[marg]))

    tmp <- reduce(IRanges(start = tmp_which_over_i+200-seq_margs [marg], end = tmp_which_over_i+200+seq_margs[marg] ))
    assign(paste0('over', seq_cutoff[i], 'IR'), tmp)
    tmp2 <- findOverlaps(tmp, expIR)
    tmp3 <- length(queryHits(tmp2))/queryLength(tmp2)
    
    df[marg, i] <- tmp3
    
    tmp4 <- paste(length(queryHits(tmp2)), '/', queryLength(tmp2))
    
    df1[marg, i] <- tmp4
  }
}

df # fraction of hits in a query
df1 #absolute values
findOverlaps(over095IR, expIR)
findOverlaps(expIR, over095IR)

#genome portion covered with high scoring sequnces
sum(over095IR@width) / nchar(e.coli_U00096.2)

#genome portion covered with experimental promoters
sum(expIR@width) / nchar(e.coli_U00096.2)

#eyeball it

par(mfrow = c(2,1))

plot(forward_strand_all_predictions, type = 'n')
abline(v = which_over095+200, col = 'darkblue')
plot(forward_strand_all_predictions, type = 'n')
abline(v = exptsss, col = 'darkred')
...
expsidds_sums <- apply(expsidds, MARGIN = 2, sum)
expsidds_means <- apply(expsidds, MARGIN = 2, mean)


plot(hist(expsidds_sums))
#bimodality check for different intervals

svg('/home/jane/Документы/Misha/mol_a_2018/bimodality_on_auc_sidds_699-exp_proms.svg', height = 9, width = 15)
par(mfrow = c (4,4),
    mar = rep(1.75,4),
    oma = rep(1.75,4))
for (i in seq(100, 750, by = 50)) {
  tmp_dataset <- expsidds[(750 - i):(750 + i),]
  tmp_expsidds_sums <- apply(tmp_dataset, MARGIN = 2, sum)
  plot(density(tmp_expsidds_sums), main = paste('Interval of', 2*i, 'nts'))
  
   mtext(text = paste('Dip test', round(dip.test(x = tmp_expsidds_sums, B = 2000 )$p.value, digits = 7)), col = 'darkred', line = 9)
}
dev.off()

plot(density(expsidds_sums))
plot(density(expsidds_means))

library(diptest)
diptest::dip.test(x = expsidds_sums, B = 2000 )
diptest::dip.test(x = expsidds_means, B = 2000 )
```
...
dend <- color_branches(as.dendrogram(hclust), col = c('black','darkred','darkblue','darkgreen','gold')[cut_hclust[order.dendrogram(as.dendrogram(hclust))]])
plot(dend%>%set('labels_cex', NA), main = 'SIDD profiles Ward clusterization', cex.main = 2)

`Sequence (nts)` <- -750:750
for (i in unique(cut_hclust[order.dendrogram(as.dendrogram(hclust))])) {
matplot(`Sequence (nts)`, t(data[which(cut_hclust==i),]), type='l', col=c('black','darkred','darkblue','darkgreen','gold')[i], main = i)
...
```{r Whether small clusters TSS overlap?}

interv <- 800


exptsss1clust <- exptsss[which(cut_ward_expsidds == 1)]
exptsss1clustIR <- IRanges(start = exptsss1clust-interv, end = exptsss1clust+interv)

exptsss2clust <- exptsss[which(cut_ward_expsidds == 2)]
exptsss2clustIR <- IRanges(start = exptsss2clust-interv, end = exptsss2clust+interv)

exptsss3clust <- exptsss[which(cut_ward_expsidds == 3)]
exptsss3clustIR <- IRanges(start = exptsss3clust-interv, end = exptsss3clust+interv)

exptsss4clust <- exptsss[which(cut_ward_expsidds == 4)]
exptsss4clustIR <- IRanges(start = exptsss4clust-interv, end = exptsss4clust+interv)


ov2_3 <- findOverlaps(exptsss2clustIR, exptsss3clustIR)
findOverlaps(exptsss2clustIR, exptsss4clustIR)
findOverlaps(exptsss3clustIR, exptsss4clustIR)
findOverlaps(exptsss4clustIR, exptsss2clustIR)

findOverlaps(exptsss2clustIR, exptsss1clustIR)
findOverlaps(exptsss3clustIR, exptsss1clustIR)
findOverlaps(exptsss4clustIR, exptsss1clustIR)

#cluster 2 and 3 share common intervals, their positions

queryHits_ov2_3 <- exptsss[queryHits(ov2_3)]

subjectHits_ov2_3 <- exptsss[subjectHits(ov2_3)]

par(mfrow= c(2,1))
plot(forward_strand_all_predictions, type = 'n')
abline(v = queryHits_ov2_3, col = 'darkblue')
plot(forward_strand_all_predictions, type = 'n')
abline(v = subjectHits_ov2_3, col = 'darkred')
