...
```{r} 
for (i in c('E01forward', 'E01reverse', 'd1forward', 'd1reverse', 'GCforward', 'GCreverse', 'EPforward', 'EPreverse')) {
  assign(i, c())
}
zout<- -480:239
...
for (i in 1:length(exp_tsss)) {
  if (exp_strands[i]=='forward') {
    E01forward<-rbind(E01forward, E01[as.numeric(exp_tsss[i]-150):(exp_tsss[i]+50)])
    #aeos2forward<-rbind(aeos2forward, matr1[(as.numericexp_tsss[i]-150):(as.numericexp_tsss[i]+50),2])
    d1forward<-rbind(d1forward, d1[(as.numeric(exp_tsss[i])-150):(exp_tsss[i]+50)])
    #aeos4forward<-rbind(aeos4forward, matr1[(as.numeric(exp_proms[i,3])-150):(as.numeric(exp_proms[i,3])+50),4])            
    GCforward<-rbind(GCforward, GC[(as.numeric(exp_tsss[i])-150):(as.numeric(exp_tsss[i])+50)])
    ...
    E01reverse<-rbind(E01reverse, E02[(as.numeric(exp_tsss[i])-50):(as.numeric(exp_tsss[i])+150)])
    #aeos2reverse<-rbind(aeos2reverse, matr1[(as.numeric(exp_tsss[i])-50):(as.numeric(exp_tsss[i])+150),2])
    d1reverse<-rbind(d1reverse, d2[(as.numeric(exp_tsss[i])-50):(as.numeric(exp_tsss[i])+150)])
    #aeos4reverse<-rbind(aeos4reverse, matr1[(as.numeric(exp_tsss[i])-50):(as.numeric(exp_tsss[i])+150),4])
    GCreverse<-rbind(GCreverse, (GC[(as.numeric(exp_tsss[i])-50):(as.numeric(exp_tsss[i])+150)]))
...
```{r Overall characteristics for both bacteria}
...
synechocystis_proms <- (readxl::read_xls('/home/mikhail/Documents/mol_a_2017/Synechocystis_cyanobacteria_prom_from_btss_article.ods', skip = 14))

```

```{r Checking whether the strain is correct}
tmp_forward <- synechocystis_proms$`1 TSS`[3]
tmp_reverse <- synechocystis_proms$`1 TSS`[1]
synechocystis_proms$`4 strand`[1:10] #3 is forward
synechocystis_proms$`6 observed sequence`[1]

substr(reverseComplement_synechocystis_NC_000911.1, start = tmp_reverse - 15, stop = tmp_reverse+15)

#synechocystis_proms$`6 observed sequence`[3]
#substr(synechocystis_NC_000911.1, start = tmp_forward - 15, stop = tmp_forward+15)


which.synechocystis_proms.fwd <- which(synechocystis_proms$`4 strand`=='fwd')
which.synechocystis_proms.rev <- which(synechocystis_proms$`4 strand`=='rev')

greped_forward <- sapply(synechocystis_proms$`6 observed sequence`[which.synechocystis_proms.fwd], FUN = function(x) {grep(x, synechocystis_NC_000911.1)})
length(greped_forward)
greped_reverse <- sapply(synechocystis_proms$`6 observed sequence`[which.synechocystis_proms.rev], FUN = function(x) {grep(x, synechocystis_NC_000911.1)})
length(greped_reverse)
length(synechocystis_proms$`1 TSS`)
```
...
for (i in c('synechocystis_E01forward', 'synechocystis_E01reverse', 'synechocystis_d1forward', 'synechocystis_d1reverse', 'synechocystis_GCforward', 'synechocystis_GCreverse', 'synechocystis_EPforward', 'synechocystis_EPreverse')) {
  assign(i, c())
}
zout<- -480:239
interval_size <- 50
for (i in 1:length(synechocystis_proms$`1 TSS`)) {
  if (synechocystis_proms$`4 strand`[i]=='fwd') {
    synechocystis_E01forward<-rbind(synechocystis_E01forward,
dynchars(synechocystis_BA000022.2_char[(synechocystis_proms$`1 TSS`[i]-150):(synechocystis_proms$`1 TSS`[i]+50)], interval_size = interval_size)$E01)                       
    synechocystis_d1forward<-rbind(synechocystis_d1forward, dynchars(synechocystis_BA000022.2_char[(synechocystis_proms$`1 TSS`[i]-150):(synechocystis_proms$`1 TSS`[i]+50)], interval_size = interval_size)$d1)                       
    #aeos4forward<-rbind(aeos4forward, matr1[(as.numeric(exp_proms[i,3])-150):(as.numeric(exp_proms[i,3])+50),4])            
    synechocystis_GCforward<-rbind(synechocystis_GCforward, dynchars(synechocystis_BA000022.2_char[(synechocystis_proms$`1 TSS`[i]-150):(synechocystis_proms$`1 TSS`[i]+50)], interval_size = interval_size)$gc1)                       
    
    p<-lseqspline1D(substr(synechocystis_BA000022.2, synechocystis_proms$`1 TSS`[i]-250, synechocystis_proms$`1 TSS`[i]+150), bound=c(50, 350), ref=251 )
...
dynchars(synechocystis_BA000022.2_char[(synechocystis_proms$`1 TSS`[i]-50):(synechocystis_proms$`1 TSS`[i]+150)], interval_size = interval_size)$E01)                       
    synechocystis_d1reverse<-rbind(synechocystis_d1reverse, dynchars(synechocystis_BA000022.2_char[(synechocystis_proms$`1 TSS`[i]-50):(synechocystis_proms$`1 TSS`[i]+150)], interval_size = interval_size)$d1)                       
    #aeos4reverse<-rbind(aeos4reverse, matr1[(as.numeric(exp_proms[i,3])-150):(as.numeric(exp_proms[i,3])+50),4])            
    synechocystis_GCreverse<-rbind(synechocystis_GCreverse, dynchars(synechocystis_BA000022.2_char[(synechocystis_proms$`1 TSS`[i]-50):(synechocystis_proms$`1 TSS`[i]+150)], interval_size = interval_size)$gc1)                       
    
    p<-lseqspline1D(substr(synechocystis_BA000022.2, synechocystis_proms$`1 TSS`[i]-150, synechocystis_proms$`1 TSS`[i]+250), bound=c(50, 350), ref=251 )
...
```{r two bacteria promoters mixture - EP}
library(fastcluster)
library(dendextend)
...
for (i in c('E01', 'd1', 'GC', 'EP')){
  ...
  ward_both_tmp<- hclust.vector(both_tmp, method = 'ward')

  colvec_ward_both_tmp <- c(rep('orange', length(exp_tsss)),rep('darkgreen',length(synechocystis_proms$`1 TSS`)))[order.dendrogram(ward_both_tmp %>% as.dendrogram )]
  ...
  plot(to_plot_ward_both_tmp, labels = F, main = paste0('E.coli and Synechocystis promoters (', i, ')'))
}
```
