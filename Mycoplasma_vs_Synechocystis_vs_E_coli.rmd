...
reverseComplement_mycoplasms_CP006916.3<- as.character(reverseComplement(DNAString(mycoplasms_CP006916.3)))
...
#cuplcl(t(synechocystis_e.coli_proms_sidds), method = 'ward', k = 4)
```
...
sidds_syn1500 <- read.csv('https://raw.githubusercontent.com/FVortex/different_bacteria_physics_and_text/master/sidds_synechocystis_1500nts.csv' )[,-1]

matplot(sidds_syn1500,ty='l',lty =1, col=1, lwd =0.5)

ward_synechocystis_1500sidd<- hclust.vector(t(sidds_syn1500), method = 'ward')
...
for (i in unique(cut_synechocystis_1500sidd)){
  matplot(`Sequence (nts)`, sidds_syn1500[, which(cut_synechocystis_1500sidd==i)], type = 'l', lty = 1, col=i)
...
```{r if dynamic properties ditributed unimodally}
