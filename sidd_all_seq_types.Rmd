...
table(maxifind_whole_e.coli_sidd_5000_1000)['1'] / sum(table(maxifind_whole_e.coli_sidd_5000_1000)) #molten genome fraction
...
svg('/home/jane/Документы/Misha/mol_a_2018/mce2018_long_sidd_maxima_vs_exp_proms.svg', height = 3, width = 25)


plot(maxifind_whole_e.coli_sidd_2500_500, type ='n', xlab = 'Genome sequence', ylab = '', yaxt = 'n', ylim = c(0,0.1), bty = 'o')
abline(v = (0:46)*250000, lty = 2, lwd = 0.5)
points(y = rep(0.066, length(which(maxifind_whole_e.coli_sidd_2500_500 ==1))), x = which(maxifind_whole_e.coli_sidd_2500_500 ==1),  pch = '|', col = 'red', ylim = c(0,1))
points(y = rep(0.033, length(exptsss)), x = exptsss,  pch = '|', col = 'darkblue')
abline(v = c(mean(rep_origin), mean(rep_terminus)), lwd = 2)

text(x = mean(rep_origin)+5e4, y = 0.01, labels = 'OriC')
text(x = mean(rep_terminus)+5e4, y = 0.01, labels = 'terC')
...
```{r Hydrophobicity of genes with frame shift, eval =F}
...
plot(ward_expsidds)
cut_ward_expsidds <- cutree(ward_expsidds, k = 4) #established previosly
...
View(genes_formula@compareClusterResult)

# #genes_formula1 <- compareCluster(Entrezid~group, data=dfGO, fun='enrichPathway', organism = 'ecolik12')

as.character(dfGO$Entrezid[which(dfGO$group==2)])
...
names(df_clust2)
View(summary(hgOver, pvalue=0.1))
```
