...
```{r three bacteria promoters mixture - all the characteristics, warning=FALSE, message=FALSE, cache=TRUE}
library(fastcluster)
library(dendextend)
cex.main = 2
...
```{r substring occurences, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r Other text representation, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r Oligonucleotide occurences, warning=FALSE, message=FALSE, cache=TRUE}
...
cuplcl(data = sidds_ecoli, method = 'ward', main = 'SIDD E. coli', k = 4)
...
cuplcl(data = sidds_syn, method = 'ward', main = 'SIDD Synechocystis sp.', k = 4)
#`Sequence (nts)` <- -750:750
#par(mfrow = c(3,2))
#plot(ward_synechocystis_1500sidd, labels = F)
#for (i in unique(cut_synechocystis_1500sidd)){
...
```{r Whole-genome SIDD vectors, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r Rollaply max, warning=FALSE, message=FALSE, cache=TRUE}
library(zoo)
...
```{r CHecking bimodality for SIDD for all promoters, warning=FALSE, message=FALSE, cache=TRUE}
