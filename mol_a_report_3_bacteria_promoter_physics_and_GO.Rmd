---
title: "Mycoplasms_vs_Synechocystis_vs_E.coli"
...
```{r Genome-wide characteristics profiels for three bacteria, cache=TRUE}
...
Creating matrices of dynamical properties and GC-content for intervals [-150; 50] nts
```{r mycoplasma proms physics, cache=TRUE} 
...
plot(ward_mycoplasma_synechocystis_e.coli_proms_stringdist, labels = F)
...
```{r Descriptive statistics for hexanucleotide frequences}
...
for (i in c('E01', 'd1', 'GC', 'EP')){
  ...
  ward_three_tmp<- hclust.vector(scale(three_tmp, center = T, scale = T), method = 'ward')

  colvec_ward_three_tmp <- c(rep('orange', length(exp_tsss)),rep('darkgreen',length(synechocystis_proms$X1.TSS)))[order.dendrogram(ward_three_tmp %>% as.dendrogram )]
  ...
  plot(to_plot_ward_three_tmp, labels = F, main = paste0('E.coli and Synechocystis promoters (', i, ')'), cex.main = cex.main )
...
cuplcl(data = t(sidds_synechocystis_1500), method = 'ward', k = 4, 'Synechocystis sp., SIDD')
...
`Sequence (nts)` <- -750:750
matplot(`Sequence (nts)`, sidds_synechocystis_1500[,synechocystis_clusters_2_3_4_numbers],ty='l',lty =1, col=1, lwd =0.5, main = 'Mycoplasma gallisepticum, 3 small clusters SIDD\n (66 / 634) promoters', ylab =  'Opening probability')
...
#cur and plot the new clusterization results
cuplcl(data = t(sidds_synechocystis_1500[,synechocystis_clusters_2_3_4_numbers]), method = 'ward', k = 3, main = 'Mycoplasma gallisepticum, 3 small clusters SIDD')
...
matplot(`Sequence (nts)`, sidds_sums_on_positions_synechocystis_3_clusters ,ty='l',lty =1, col=1, lwd = 1, main = 'Synechocystis gallisepticum, 3 small clusters SIDD\n (66 / 634) promoters', ylab =  'Opening probability')
...
matplot(`Sequence (nts)`, sidds_sums_on_positions_synechocystis_all_clusters ,ty='l',lty =1, col=1, lwd = 1, main = 'Synechocystis gallisepticum, 3 small clusters SIDD\n (66 / 634) promoters', ylab =  'Opening probability')

```


```{r cluster analysis for various properties and 3 bacteria, E01}
library(fastcluster)
...
#an interesting cluster for Mycoplasma E01
...
ggseqlogo(mycoplasma_peculiar_E01cluster_short_seqs,  ncol = 1)+ scale_y_continuous(name="Информационнное содержание (бит)")+
scale_x_discrete(name ="Последовательность относительно ТСТ (п.н.)", limits=1:100, labels = -50:50)
...
scale_x_discrete(name ="Последовательность относительно ТСТ (п.н.)", limits=1:200, labels = -150:50)
#does the cluster representatives differ from the universe?
...
```{r Same for scaled versions}
cuplcl(t(scale(t(E01), center = T, scale = T)), method = 'ward', k = 4)
cuplcl(t(scale(t(synechocystis_E01), center = T, scale = T)), method = 'ward', k = 4)
cuplcl(t(scale(t(mycoplasma_E01), center = T, scale = T)), method = 'ward', k = 4)

```

```{r cluster analysis for various properties and 3 bacteria, EP}
library(fastcluster)
...
#an interesting cluster for Mycoplasma EP
...
```{r Same for scaled versions}
cuplcl(t(scale(t(EP), center = T, scale = T)), method = 'ward', k = 4)
cuplcl(t(scale(t(synechocystis_EP), center = T, scale = T)), method = 'ward', k = 4)
cuplcl(t(scale(t(mycoplasma_EP), center = T, scale = T)), method = 'ward', k = 4)

```



```{r cluster analysis for various properties and 3 bacteria, GC}
library(fastcluster)
...
cuplcl(GC, method = 'ward', k = 4, main = 'E. coli, GC')
cuplcl(synechocystis_GC, method = 'ward', k = 4, main = 'Synechocystis, GC')
cuplcl(mycoplasma_GC, method = 'ward', k = 4, main = 'Mycoplasma gallisepticum, GC')

```

```{r Same for scaled versions}
cuplcl(t(scale(t(GC), center = T, scale = T)), method = 'ward', k = 4)
cuplcl(t(scale(t(synechocystis_GC), center = T, scale = T)), method = 'ward', k = 4)
cuplcl(t(scale(t(mycoplasma_GC), center = T, scale = T)), method = 'ward', k = 4)

```


```{r Reading in transription units data}
...
```{r E01 clusters population and GOstats}
...
```{r GO statistics using GOstats for E01}
library(GOstats)
library(Category)
library(DBI)
library(org.EcK12.eg.db)
...
View(df_summary_clust1) #primarly nucleoside metabolisim and ion transport
...
```{r cluster analysis for various properties and 3 bacteria, d1}
library(fastcluster)
...
cuplcl(d1, method = 'ward', k = 4, main = 'E. coli, d1')
cuplcl(synechocystis_d1, method = 'ward', k = 4, main = 'Synechocystis, d1')
cuplcl(mycoplasma_d1, method = 'ward', k = 4, main = 'Mycoplasma gallisepticum, d1')

```

Promoter numbers and genenames
```{r d1 clusters population and GOstats}
...
```{r GO statistics using GOstats for d1}
library(GOstats)
library(Category)
library(DBI)
library(org.EcK12.eg.db)
