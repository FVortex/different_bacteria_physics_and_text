...
```{r, code=readLines("/home/mikhail/Documents/mol_a_2017/new_physics_functions.R")}
```
...
```{r  Reading in M. gallisepticum strain S6 promoters from Fisunov et al., 2016}
...
complement_extended_mycoplasma_CP006916.3 <- Biostrings::complement(DNAString(extended_mycoplasma_CP006916.3))           
reverseComplement_extended_mycoplasma_CP006916.3 <- reverseComplement(DNAString(extended_mycoplasma_CP006916.3))           
...
for (i in 1:length(mycoplasma_proms$TSS)) {
  if (mycoplasma_proms$strand[i]=='forward') {
   ...
   # #! note all TSS are shifted
    ...
    mycoplasma_seqsreverse <- c(mycoplasma_seqsreverse, get_forward_subseq(seq = complement_extended_mycoplasma_CP006916.3, tss = mycoplasma_proms$TSS[i]+ extended_by, boundaries = c(150, 50)))
    
    seq_tmp_reverse <- strsplit(get_forward_subseq(seq = complement_extended_mycoplasma_CP006916.3, tss = mycoplasma_proms$TSS[i]+ extended_by, boundaries = c(interv, interv-2)), split = '')[[1]] #used in profiles calculation
...
plot(to_plot_ward_mycoplasma_synechocystis_e.coli_proms_stringdist, main = 'E. coli, Synechocystis sp., and Mycoplasma gallisepticum promoters: Levenshtein distance', cex.main = cex.main )
...
colnames(all_seqs_oligos_freq) <- c('E. coli\n promoters', 'Synechocystis sp.\n promoters', 'Mycoplasma gallisepticum\n promoters', 'E. coli\n complete genome', 'Synechocystis sp.\n complete genome', 'Mycoplasma gallisepticum\n complete genome')
...
cuplcl(ecoli_E01, method = 'ward', k = 3, main = 'E. coli, E01')
cuplcl(synechocystis_E01, method = 'ward', k = 4, main = 'Synechocystis, E01')
cuplcl(mycoplasma_E01, method = 'ward', k = 3, main = 'Mycoplasma gallisepticum, E01')
...
cuplcl(t(scale(t(synechocystis_E01), center = T, scale = T)), method = 'ward', main = 'Scaled E01, Synechocystis', k = 3)
cuplcl(t(scale(t(mycoplasma_E01), center = T, scale = T)), method = 'ward', main = 'Scaled E01, Mycoplasma gallisepticum', k = 3)
...
cuplcl(ecoli_EP, method = 'ward', k = 3, main = 'E. coli, EP')
...
cuplcl(t(scale(t(ecoli_EP), center = T, scale = T)), method = 'ward', main = 'Scaled EP, E. coli', k = 5)
cuplcl(t(scale(t(synechocystis_EP), center = T, scale = T)), method = 'ward', main = 'Scaled EP, Synechocystis sp.', k = 3)
cuplcl(t(scale(t(mycoplasma_EP), center = T, scale = T)), method = 'ward', main = 'Scaled EP, Mycoplasma gallisepticum', k = 4)
...
cuplcl((ecoli_GC), method = 'ward', k = 4, main = 'E. coli, GC')
...
cuplcl(t(scale(t(synechocystis_GC), center = T, scale = T)), method = 'ward', main = 'Scaled GC, Synechocystis', k = 3)
cuplcl(t(scale(t(mycoplasma_GC), center = T, scale = T)), method = 'ward', main = 'Scaled GC, Mycoplasma gallisepticum', k = 3)
...
plot(ward_mycoplasma_proms_stringdist, labels = F, xlab = '', main ='Mycoplasma gallisepticum, levenshtein string distance') #k =6
...
plot(seq_along(mycoplasma_CP006916.3_char), type = 'n', ylim = 0:1, main ='Mycoplasma gallisepticum levenstien distance, 2 small clusters (both 13 members) positions', asp = 2)
...
names(tmps) <- c('Mycoplasma gallispeticum, string distance major cluster', 'Small cluster 1', 'Small cluster 2')
# to labelso only certain positions
vec <- rep('', length(-150:50))
labels <- -150:50
vec[seq(1, length(-150:50), 25)] <- labels[seq(1, length(-150:50), 25)]

ggseqlogo(tmps,  ncol = 1)+ scale_y_continuous(name="Информационнное содержание (бит)")+scale_x_discrete(name ="Последовательность относительно ТСТ (п.н.)", limits=1:201, labels = vec)+coord_fixed(ratio = 10)
```
...
cut_ward_ecoli_E01 <- cutree(ward_ecoli_E01, k = 3)
...
cut_ward_ecoli_EP <- cutree(ward_ecoli_EP, k = 3)
...
cut_ward_ecoli_GC <- cutree(ward_ecoli_GC, k = 3)
